...
void Liver::show()
{
	wcout << L"Имя: " << name << L"  ";// << L"  Номер дома: " << homeNum << L"  Почтовый индекс: " << postNum << L"  ";
}
void Liver::input()
{
...
Emploee::Emploee(wchar_t* n, wchar_t* j, int w, int s) :Liver(n, 0 , 0)
{
	wcscpy_s(job, j);
	wages = w;
	seniority = s;
...
{
	Liver::show();
	wcout << L"\tПрофессия: " << job << L"\tЗаработная плата: " << wages << L"  Стаж работы: " << seniority << L" ";// << L"  Дата начала работы: " << beginW.getDay() << L"." << beginW.getMonth() << L"." << beginW.getYear() << L" ";
}
void Emploee::input()
{
	Liver::input();
...
Emploee Emploee::operator[](int i)
{
	return *this;
}
//--------------------------------------------------------------------//
...
void indinsert(Emploee *&x, Emploee y, int &size, int &n, int ind)
{
	for (int i = 0; i < n; i++)
	{
		if (i == ind)
...
void MoveS(Emploee *&x, int &size, int &n, int ind)
{
	if (n < size)
	{
...
void push_back(Emploee *&y, int &n, int &size, Emploee x)
{
...
size_t Capacity(size_t g, int n)
{
	return g - n;
}
void Resize(Emploee *&x, int newsize, int &size, int &n)
{
	if (newsize > Capacity(size, n))
	{
		Emploee *data;
		data = new Emploee[newsize];
		for (int i = 0; i < n; i++)
		{
			data[i] = x[i];
		}
...
void fill(Emploee *&x, int &size, int &n, int k)
{
	Emploee e;
	//srand(time(0));
	for (int i = 0; i < k; i++)
	{
		...
		e.setSeniority(rand() % 10);
		e.setName(names[rand() % Const]);
		push_back(x, n, size, e);
	}
}
void insert(Emploee *& x, int &n, Emploee y, int& size, int ind, int m)
{
	for (int i = 0; i < n; i++)
	{
		if (i == ind)
...
void Sortname(Emploee *&x, binary_func k, int n)
{
	for (int i = 0; i < n - 1; i++)
	{
		for (int j = i + 1; j < n; j++)
		{
			if (k((wchar_t*)x[i].getName(), (wchar_t*)x[j].getName()) == k.type)
			{
				swap(x[i], x[j]);
			}
		}
...
void Sortwages(Emploee *&x, binary_func k, int n)
{
	for (int i = 0; i < n - 1; i++)
	{
		for (int j = i + 1; j < n; j++)
		{
			if (k(x[i].getWages(), x[j].getWages()) == k.type)
			{
				swap(x[i], x[j]);
			}
		}
...
void Sortseniority(Emploee *&x, binary_func k, int n)
{
	for (int i = 0; i < n - 1; i++)
	{
		for (int j = i + 1; j < n; j++)
		{
			if (k(x[i].getSeniority(), x[j].getSeniority()) == k.type)
			{
				swap(x[i], x[j]);
			}
		}
...
void Sortjob(Emploee *&x, binary_func k, int n)
{
	for (int i = 0; i < n - 1; i++)
	{
		for (int j = i + 1; j < n; j++)
		{
			if (k((wchar_t*)x[i].getJob(), (wchar_t*)x[j].getJob()) == k.type)
			{
				swap(x[i], x[j]);
			}
		}
