...
Organization& Organization_Ukaz::operator=(const Organization& a)
{
...
void Organization_Mas::Del_Wages(int a)
{
    for(int i=0;i<N_Emploee;i++)
    {
        if(p[i].getWages()==a)
...
void Organization_Mas::Del_Seniority(int a)
{
    for(int i=0;i<N_Emploee;i++)
    {
        if(p[i].getSeniority()==a)
...
void Organization_Mas::Del_Data(Data a)
{
    for(int i=0;i<N_Emploee;i++)
    {
        if(((p[i].getData().getDay())==a.getDay())&&((p[i].getData().getMonth())==a.getMonth())&&((p[i].getData().getYear())==a.getYear()))
...
void Organization_Mas::Del_Job(wstring a)
{
    for(int i=0;i<N_Emploee;i++)
    {
        if(p[i].getJob()==a)
...
void Organization_Mas::Del_Name(wstring a)
{
    for(int i=0;i<N_Emploee;i++)
    {
        if(p[i].getName()==a)
...
void Organization_Mas::Del_HomeNum(int a)
{
    for(int i=0;i<N_Emploee;i++)
    {
        if(p[i].getHomeNum()==a)
...
void Organization_Mas::Del_PostNum(int a)
{
    for(int i=0;i<N_Emploee;i++)
    {
        if(p[i].getPostNum()==a)
...
Organization& Organization_Mas::operator=(const Organization& a)
{
    if(this==&a){return *this;}
    else
    {
	...
	while (h != L"stop")
	{
		...
		else if (h == L"help")
		{
			wcout << L"add - добавление элемента" << endl
				...
				<< L"delhomenum - удаление дубликатов по количеству работников" << endl
				<< L"delpostnum - удаление дубликатов по количеству касс" << endl
				<< L"deljob - удаление дубликатов по типу" << endl
				<< L"delseniority - удаление дубликатов по типу" << endl
				<< L"delwages - удаление дубликатов по типу" << endl
				<< L"delbeginw - удаление дубликатов по типу" << endl
				<< L"fill - заполнение массива" << endl
				...
				<< L"stop - сортировка по типу" << endl;
		}
		else if (h == L"fill")
		{
